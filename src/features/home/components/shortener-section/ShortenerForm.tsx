...
export function ShortenerForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!isURL(originalLink)) {
      ...
      createShortLink(originalLink).then(res => {
        if (res.error) {
  ...
  return (
    <motion.form className="relative border p-4 rounded-xl border-graphite bg-neutral-950/70 backdrop-blur-sm xs:p-5 sm:p-7
